...
do
  ...
  if [ `date +%d` -eq $d ]; then echo `date`": "$TEXT >> daily.txt; else curl -F file=@daily.txt -F "initial_comment=本日のサマリー" -F channels=nohara-demo -H "Authorization: Bearer "$BOT https://slack.com/api/files.upload && rm -rf daily.txt && d=`date +%d`; fi
  sleep $ITV
done
